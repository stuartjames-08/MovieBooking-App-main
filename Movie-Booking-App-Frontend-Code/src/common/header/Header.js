...
class Header extends Component {
    ...
    loginClickHandler = () => {
        ...
        xhrLogin.addEventListener("readystatechange", function () {
            if (this.readyState === 4) {
                sessionStorage.setItem("uuid", JSON.stringify(this.responseText).email);
                sessionStorage.setItem("access-token", xhrLogin.getResponseHeader("access-token"));

                if (xhrLogin.getResponseHeader("access-token") == null) {
                    sessionStorage.setItem("access-token", JSON.stringify(this.responseText)["access-token"]);
                }
    ...
    logoutHandler = () => {
        ...
        if (!uuid) {
            // If there is no UUID in sessionStorage, the user is not logged in.
            // You might want to handle this case accordingly.
            return;
        }

        const dataSignout = JSON.stringify({ "uuid": uuid });
        ...
        fetch(this.props.baseUrl + "auth/logout", {
            ...
            .then(data => {
                if (data.message === "Logged Out successfully.") {
                    ...
                    // Handle unsuccessful logout response here if needed.
                }
            })
            .catch(error => {
